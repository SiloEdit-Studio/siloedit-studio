        ...
        try {
            ...
            if (isAiEnhance || isCleanup || scale > 1) {
                ...
                val aiResult = upscaler.processPipeline(
                    ...
                    targetScale = scale,
                    onProgress = { msg, prog ->
                        updateNotification(msg, 20 + (prog * 0.4).toInt(), 100)
                    }
                )
                if (aiResult != null) {
            ...
            // 2. DESENFOQUE (Cloud o InSPyReNet Local) - AHORA DESPUÉS DEL ESCALADO
            if (blur > 0f) {
                if (useCloud) {
                    updateNotification("PhotoRoom Matting...", 70, 100)
                    val cloudResult = processPhotoRoomMatting(bitmap)
                    if (cloudResult != null) {
                        val processor = ImageProcessor()
                        val resolutionScale = bitmap.width / 1024f
                        val effectiveBlur = blur * resolutionScale
                        val blurred = processor.applySelectiveBlur(
                            bitmap, 
                            cloudResult, 
                            effectiveBlur, 
                            isNaturalDepth = true, 
                    ...
                    updateNotification("IA Portrait Matting...", 70, 100)
                    val segmentation = AiSegmentation(this)
                    if (segmentation.loadModel()) {
                        val mask = segmentation.getPersonMask(bitmap)
                        if (mask != null) {
                            val processor = ImageProcessor()
                            // Escalado de radio de desenfoque proporcional a la resolución (Base 1024px)
                            val resolutionScale = bitmap.width / 1024f
                            val effectiveBlur = blur * resolutionScale
                            
                            Log.d("ExportService", "Aplicando desenfoque: radius=$effectiveBlur (orig=$blur) en ${bitmap.width}x${bitmap.height}")
                            ...
                            Log.e("ExportService", "No se detectó sujeto para el desenfoque.")
                        }
                    }
                    segmentation.close()
                }
